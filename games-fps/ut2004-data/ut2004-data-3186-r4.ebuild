...
EAPI=7
inherit eutils unpacker cdrom portability check-reqs
...
S=${WORKDIR}
dir=/opt/ut2004 # possible cleanup here? Addons all install to /usr/share/games/ut2004/
Ddir=${D}/${dir}

CHECKREQS_DISK_BUILD="7G"
CHECKREQS_DISK_USR="7G" # actually installs to /opt/, but this should be approximate
...
check_dvd() {
	...
	while read -r point foo fs foo ; do
...
src_install() {
	...
	if [[ ${USE_MIDWAY_DVD} -eq 1 ]] ; then
		...
		local uz2_src="${Ddir}/${j}" qa_uz2_did_dirs=0 qa_uz2_did_files=0
		for j in Animations Maps Sounds StaticMeshes Textures ; do
			fperms -R u+w "${dir}/${j}" || die

			# open-coded copy of games_ut_unpack from games.eclass
			if [[ -f ${uz2_src} ]]; then
				qa_uz2_did_dirs=1
				uz2unpack "${uz2_src}" "${uz2_src%.uz2}" \
					|| die "uncompressing file ${uz2_src}"
			fi
			if [[ -d ${uz2_src} ]]; then
				qa_uz2_did_files=1
				while read -r unpack_f; do
					uz2unpack "${uz2_src}/${unpack_f}" "${uz2_src}/${unpack_f%.uz2}" \
						|| die "uncompressing file ${unpack_f}"
					rm -f "${uz2_src}/${unpack_f}" \
						|| die "deleting compressed file ${unpack_f}"
				done < <(find "${uz2_src}" -maxdepth 1 -name '*.uz2' -printf '%f\n' 2>/dev/null)
			fi
		done
		if ! (( qa_uz2_did_dirs )); then
			eqawarn "Unpack wasn't called for any directories"
		fi
		if ! (( qa_uz2_did_files )); then
			eqawarn "Unpack wasn't called for any files"
		fi
	fi

	# Create empty files in Benchmark
	for keepdir_f in CSVs Logs Results; do
		keepdir "${dir}"/Benchmark/${keepdir_f}
	done
