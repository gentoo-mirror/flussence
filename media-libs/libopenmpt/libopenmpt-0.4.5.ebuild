...
IUSE="cli examples flac modplug-compat modplug +mp3 portaudio pulseaudio sdl sdl2 sndfile +vorbis +zlib"
...
src_configure() {
	econf \
		"$(use_enable cli openmpt123)" \
			...
			"$(use_with sndfile)" \
		"$(use_enable examples)" \
		"$(use_enable modplug-compat libopenmpt_modplug)" \
		"$(use_enable modplug libmodplug)" \
		"$(use_with mp3 mpg123)" \
		"$(use_with vorbis ogg)" \
		"$(use_with vorbis)" \
